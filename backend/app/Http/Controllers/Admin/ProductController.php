...
class ProductController extends Controller
{
    ...
    public function store(AddProductRequest $request)
    {
        ...
        if($request->validated()) {
            ...
            //check if the admin actually uploaded a first image
            if($request->hasFile('first_image')) {
                $data['first_image'] = $this->saveImage($request->file('first_image'));
            }
            //check if the admin actually uploaded a second image
            if($request->hasFile('second_image')) {
                $data['second_image'] = $this->saveImage($request->file('second_image'));
            }
            //check if the admin actually uploaded a third image
    ...
    public function update(UpdateProductRequest $request, Product $product)
    {
        ...
        if($request->validated()) {
            ...
            if($request->hasFile('thumbnail')) {
                ...
                //no new thumbnail uploaded - don't overwrite the existing one with null
                unset($data['thumbnail']);
            }
            //check if the admin actually uploaded a new first image
            if($request->hasFile('first_image')) {
            ...
            //check if the admin actually uploaded a new second image
            if($request->hasFile('second_image')) {
            ...
            //check if the admin actually uploaded a new third image
            if($request->hasFile('third_image')) {
